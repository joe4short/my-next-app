...
const MyForm: React.FC<{ onMessage: (msg: string | null) => void }> = ({ onMessage }) => {
  const handleSubmit = async (values: FormValues) => {
    try {
      const response = await axios.post('/api/registration', values);
      onMessage(response.data.message || 'Registration successful');
    } catch (error: any) {
      onMessage(error.response?.data?.error || 'Registration failed');
    }
  };
